...
class TaqDaily:
    ...
    def compute_spreads(
        ...
        df.loc[sel, "inforce"] = np.abs(
            (
                datetime.combine(date, end_time_spreads) - df.loc[sel, "timestamp"]
            ).dt.total_seconds()
        )

        # Delete locked and crossed quotes
        sel = (df.best_bid == df.best_ask) | (df.best_bid > df.best_ask)
        df = df[~sel]
        ...
        spreads_df = df.groupby("symbol")[measures + ["inforce"]].apply(
            compute_wspreads
        )
        return spreads_df
    ...
    def compute_rs_and_pi(
        ...
        if off_nbbo_df is None:
            off_nbbo_df = self.get_official_complete_nbbo(date=date, symbols=symbols)
        if trade_and_nbbo_df is None:
            trade_and_nbbo_df = self.merge_trades_nbbo(
                date=date, symbols=symbols, off_nbbo_df=off_nbbo_df
            )

        if track_retail is None:
            track_retail = self.track_retail
        next_df = off_nbbo_df[["timestamp", "symbol", "best_bid", "best_ask"]].copy()
        next_df["midpoint"] = (next_df["best_bid"] + next_df["best_ask"]) / 2
        next_df["timestamp"] = next_df["timestamp"] - delay

        trade_and_nbbo_df = trade_and_nbbo_df.sort_values(["timestamp", "symbol"])
        ...
        for sign in signs:
            ...
            df["PercentRealizedSpread_" + sign + suffix] = (
                df["BuySell" + sign]
                * (np.log(df["price"]) - np.log(df["midpoint_next"]))
                * 2
            )
            df["DollarPriceImpact_" + sign + suffix] = (
                df["BuySell" + sign] * (df["midpoint_next"] - df["midpoint"]) * 2
            )
            df["PercentPriceImpact_" + sign + suffix] = (
                df["BuySell" + sign]
                * (np.log(df["midpoint_next"]) - np.log(df["midpoint"]))
                * 2
            )
    ...
    def compute_averages_ave_sw_dw(
        self, df, measures, simple=True, dollar_weighted=True, share_weighted=True
    ):
        weights = []
        if dollar_weighted:
            weights.append("dollar")
        if share_weighted:
            weights.append("size")
